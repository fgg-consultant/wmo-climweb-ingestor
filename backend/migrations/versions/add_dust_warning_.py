...
def upgrade():
    ...
    with op.batch_alter_table('geo_region', schema=None) as batch_op:
        batch_op.create_index('idx_geo_region_geom', ['geom'], unique=False, postgresql_using='gist', postgresql_ops={})
    ...
    op.create_table('aemet_dust_warning',
                    ...
                    sa.ForeignKeyConstraint(['gid'], ['geo_region.gid'], ondelete='CASCADE'),
                    sa.PrimaryKeyConstraint('id'),
                    sa.UniqueConstraint('gid', 'init_date', 'forecast_date', name='unique_dust_warning_date')
                    )

    op.create_table('aemet_dust_forecast',
                    sa.Column('id', sa.Integer(), nullable=False),
                    sa.Column('gid', sa.String(length=256), nullable=False),
                    sa.Column('init_date', sa.DateTime(), nullable=False),
                    sa.Column('forecast_date', sa.DateTime(), nullable=False),
                    ...
                    sa.Column('geom', Geometry(geometry_type='MULTIPOLYGON', srid=4326, spatial_index=False, from_text='ST_GeomFromEWKT', name='geometry'), nullable=False),
                    sa.PrimaryKeyConstraint('id'),
                    sa.UniqueConstraint('gid', 'init_date', 'forecast_date', name='unique_dust_forecast_date')
                    )

def downgrade():
    op.drop_table('aemet_dust_warning')
    op.drop_table('aemet_dust_forecast')

    with op.batch_alter_table('geo_region', schema=None) as batch_op:
        batch_op.drop_index('idx_geo_region_geom', postgresql_using='gist', column_name='geom')
