---
outline: deep
---

# Natural Colour Enhanced RGB

![An image](/wms/msg_fes_rgb_naturalenhncd.png)

## Dataset

| **Input**                               |             **Value**              |
| --------------------------------------- | :--------------------------------: |
| **Title**                               |     Natural Color Enhanced RGB     |
| **Category**                            |             Satellite              |
| **Subcategory**                         |           Near Realtime            |
| **Layer Type**                          |    Web Map Service – WMS Layer     |
| **Summary**                             | EUMETSAT, Updated every 15 minutes |
| **Metadata**                            |                                    |
| **Published**                           |                 ✅                 |
| **Public**                              |                 ✅                 |
| **Initially Visible on Map by default** |                 ✅                 |
| **Multi-temporal**                      |                 ✅                 |
| **Multi-layer**                         |                 ❌                 |
| **Near realtime**                       |                 ✅                 |
| **Current time method**                 | Latest available date from source  |
| **Auto Update interval in minutes**     |                 15                 |
| **Enable Clipping by shape**            |                 ❌                 |

## Layer

| **Input**                                |         **Value**          |
| ---------------------------------------- | :------------------------: |
| **Dataset**                              | Natural Color Enhanced RGB |
| **Title**                                | Natural Color Enhanced RGB |
| **Default**                              |             ✅             |
| **Display Format for DateTime Selector** |     Hour minute:second     |

## WMS GetMap Configuration

| **Input**                    |                **Value**                |
| ---------------------------- | :-------------------------------------: |
| **Base url for WMS**         | https://view.eumetsat.int/geoserver/wms |
| **WMS Version**              |                  1.3.0                  |
| **Pixel Width**              |                   256                   |
| **Pixel Height**             |                   256                   |
| **Transparency**             |                   ✅                    |
| **Spatial Reference System** |                EPSG:3857                |
| **Output Format**            |                   PNG                   |

## WMS Request Layers

| **Input**                    |         **Value**         |
| ---------------------------- | :-----------------------: |
| **WMS Request Layer 1 Name** | msg_fes:rgb_naturalenhncd |

## WMS GetCapabilities Configuration

| **Input**                          |                             **Value**                             |
| ---------------------------------- | :---------------------------------------------------------------: |
| **Request Time from capabilities** |                                ✅                                 |
| **Get Capabilities URL**           | https://view.eumetsat.int/geoserver/msg_fes/rgb_naturalenhncd/ows |
| **Get Capabilities Layer Name**    |                         rgb_naturalenhncd                         |

## Legend

No legend image provided in markdown. See PDF for details.

## More Info

- [Learn More](https://data.eumetsat.int/product/EO:EUM:DAT:MSG:NCL_ENH): The Natural Colour Enhanced product is an RGB which utilises three SEVIRI solar channels: NIR1.6, VIS0.8 and VIS0.6.
